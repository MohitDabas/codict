...
import re
import requests
with open('javascriptlangdb.json') as fp:
    data =json.load(fp)
def javascriptsendSuggestion(functionName):
    global data
    tenSuggestion=[]
...
def javascriptDescribeRequest(functionName,path):
    headers = {
    ...
    path=path+'.html'
    print('https://docs.devdocs.io/javascript/'+path)
    response = requests.get('https://docs.devdocs.io/javascript/'+path, 
    headers=headers)
    data=response.text
    return data